/*
 * Copyright (C) 2023 thevalidator
 */

package ru.thevalidator.tictactoe.gui;

/**
 * @author thevalidator
 */
public class Settings {
    
    public static final int BOX_SIZE = 160;
    public static final int MARGIN = 15;
    public static final int MAIN_WINDOW_WIDTH = BOX_SIZE * 3 + MARGIN * 4;
    public static final int MAIN_WINDOW_HEIGHT = MAIN_WINDOW_WIDTH + 100;
    
    
    //WIN COLOR #ED1F1F
    //CIRCLE COLOR #AFAFAF
    //CROSS COLOR #58618A
    
    //<a target="_blank" href="https://icons8.com/icon/eTofvMVD4UUm/circle">Circle</a> icon by <a target="_blank" href="https://icons8.com">Icons8</a>
    //<a target="_blank" href="https://icons8.com/icon/RvdYzOup1Xqx/multiply">Multiply</a> icon by <a target="_blank" href="https://icons8.com">Icons8</a>
    //<a target="_blank" href="https://icons8.com/icon/vvHsCpLe200a/horizontal-line">Horizontal Line</a> icon by <a target="_blank" href="https://icons8.com">Icons8</a>

}
